// SPDX-License-Identifier: GPL-2.0-or-later
// Copyright The XCSoar Project

#include "Dialogs/DialogSettings.hpp"
#include "Dialogs/SimulatorPromptWindow.hpp"
#include "Dialogs/WidgetDialog.hpp"
#include "Widget/WindowWidget.hpp"
#include "UIGlobals.hpp"
#include "Look/DialogLook.hpp"
#include "ui/window/Init.hpp"
#include "Screen/Layout.hpp"
#include "ui/event/KeyCode.hpp"
#include "../test/src/Fonts.hpp"
#include "Language/Language.hpp"
#include "ui/window/SingleWindow.hpp"
#include "ui/canvas/Canvas.hpp"
#include "DisplayOrientation.hpp"
#include "System.hpp"
#include "UtilsSystem.hpp"
#include "NetworkDialog.hpp"
#include "SystemDialog.hpp"
#include "ToolsDialog.hpp"

enum Buttons {
  TOOLS = 100,
  NETWORK,
  SYSTEM,
  POWEROFF
};

static DialogSettings dialog_settings;
static UI::SingleWindow *global_main_window;
static DialogLook *global_dialog_look;

const DialogSettings &
UIGlobals::GetDialogSettings()
{
  return dialog_settings;
}

UI::SingleWindow &
UIGlobals::GetMainWindow()
{
  assert(global_main_window != nullptr);

  return *global_main_window;
}

const DialogLook &
UIGlobals::GetDialogLook()
{
  assert(global_dialog_look != nullptr);

  return *global_dialog_look;
}

class ColibriMenuWidget final : public WindowWidget {
  WndForm &dialog;
  SimulatorPromptWindow::Result initial_result;

public:
  ColibriMenuWidget([[maybe_unused]] const DialogLook &_look,
                 WndForm &_dialog,
                 SimulatorPromptWindow::Result _initial_result)
    :dialog(_dialog), initial_result(_initial_result) {}

  void CreateButtons(WidgetDialog &buttons);

  /* virtual methods from class Widget */
  void Prepare(ContainerWindow &parent,
               const PixelRect &rc) noexcept override;
  bool KeyPress(unsigned key_code) noexcept override;
};

void
ColibriMenuWidget::CreateButtons(WidgetDialog &buttons)
{
  buttons.AddButton(_("Tools"), [](){ ShowToolsDialog(); });
  buttons.AddButton(_("Network"), [](){ ShowNetworkDialog(); });
  buttons.AddButton(_("System"), [](){ ShowSystemDialog(); });
  buttons.AddButton(_("Poweroff"), dialog.MakeModalResultCallback(POWEROFF));
}

void
ColibriMenuWidget::Prepare(ContainerWindow &parent,
                        const PixelRect &rc) noexcept
{
  WindowStyle style;
  style.Hide();
  style.ControlParent();

  auto w = std::make_unique<SimulatorPromptWindow>(dialog.GetLook(),
                                                   [this](SimulatorPromptWindow::Result result){
                                                     dialog.SetModalResult(int(result));
                                                   }, false);
  w->SetInitialResult(initial_result);
  w->Create(parent, rc, style);
  SetWindow(std::move(w));
}

bool
ColibriMenuWidget::KeyPress([[maybe_unused]] unsigned key_code) noexcept
{
  return false;
}

static int
Main(UI::SingleWindow &main_window, const DialogLook &dialog_look,
     SimulatorPromptWindow::Result initial_result)
{
  TWidgetDialog<ColibriMenuWidget>
    dialog(WidgetDialog::Full{}, main_window,
           dialog_look, nullptr);
  dialog.SetWidget(dialog_look, dialog, initial_result);
  dialog.GetWidget().CreateButtons(dialog);

  return dialog.ShowModal();
}

static int
Main(SimulatorPromptWindow::Result initial_result)
{
  dialog_settings.SetDefaults();

  ScreenGlobalInit screen_init;

  PixelSize size = SystemWindowSize();
  if (size.width == 0)
    size = {800, 480};

  UI::TopWindowStyle main_style;
  main_style.Resizable();
#ifdef SOFTWARE_ROTATE_DISPLAY
  main_style.InitialOrientation(DisplayOrientation::PORTRAIT);
#endif

  UI::SingleWindow main_window{screen_init.GetDisplay()};
  main_window.Create("XCSoar/ColibriMenu", size, main_style);

  size = main_window.GetSize();

  Layout::Initialise(screen_init.GetDisplay(), size);
  InitialiseFonts();

  DialogLook dialog_look;
  dialog_look.Initialise();

  main_window.Show();

  global_dialog_look = &dialog_look;
  global_main_window = &main_window;

  int action = Main(main_window, dialog_look, initial_result);

  main_window.Destroy();

  DeinitialiseFonts();

  return action;
}

int main([[maybe_unused]] int argc, [[maybe_unused]] char **argv)
{
  ApplyColibriWifiAutoOn();

  SimulatorPromptWindow::Result last_result = SimulatorPromptWindow::Result::FLY;

  while (true) {
    int action = Main(last_result);

    switch (action) {
    case int(SimulatorPromptWindow::Result::FLY):
      last_result = SimulatorPromptWindow::Result::FLY;
      ColibriRunXCSoar("-fly");
      /* return to menu after XCSoar quits */
      break;

    case int(SimulatorPromptWindow::Result::SIMULATOR):
      last_result = SimulatorPromptWindow::Result::SIMULATOR;
      ColibriRunXCSoar("-simulator");
      /* return to menu after XCSoar quits */
      break;

    case POWEROFF:
      ColibriPowerOff();
      return EXIT_SUCCESS;

    default:
      return EXIT_SUCCESS;
    }
  }
}
